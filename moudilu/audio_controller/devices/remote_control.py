...
class RemoteControl:
    ...
    the event from there is emitted. If no event is in the other dictionary for this key,
    the event from this KEY_EVENTS dictionary is emitted even if the keypres was long."""
    ...
    def __init__(self, device_number: int = 0) -> None:
        ...
        """For key currently pressed that have registered longpress events, holds the timestamp it had been pressed"""
    ...
    async def _read_keys(self) -> None:
        async for ev in self.device.async_read_loop():
            ...
            if ev.keystate == ev.key_down and ev.keycode in self.KEY_EVENTS_LONGPRESS:
                # There is an event for a longpress for this key, track how long it is pressed
                self._key_down_timestamps[ev.keycode] = ev.event.timestamp()

            elif ev.keystate == ev.key_up:
                # A key has been released - emit the corresponding event if available
                if (
                    ev.keycode in self._key_down_timestamps
                    and (ev.event.timestamp() - self._key_down_timestamps[ev.keycode])
                    > self.LONGPRESS_THRESHOLD
                ):
                    # the key was pressed for long and there is an longpress event registered, fire it
                    self._logger.info("%s pressed", ev.keycode)
                    self._router.fire_event(
